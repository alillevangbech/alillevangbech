...
def get_lr(file_path):
	df = pd.read_csv(file_path)
	print(float(df.columns[0]))
	return float(df.columns[0])
...
class PartialAutomation(tf.keras.callbacks.Callback):
    ...
    def on_train_begin(self, logs=None):
        ...
        self.batch_size = get_batch_size(self.batch_path)        
        ## Insert learning rate here
        
        temp_csv = get_best_error(self.best_error_path)
        if not temp_csv == -1:
            self.best_error = temp_csv
        else:
            self.best_error = 9999999
        
        temp_csv = get_batch_size(self.wait_path)
        if not temp_csv == 0:
            self.wait = temp_csv
        else:
            self.wait = 0
        
        self.patience = get_batch_size(self.patience_path)
        self.percent_range = get_lr(self.percent_range_path)    
        self.last_error = 0
        self.skip_first = 10
    ...
    def on_epoch_end(self,epoch,logs=None):
        ...
        ###############################################
        temp_command = checkpoint(self.stop_path)
        if (temp_command == "stop" or temp_command == "reset"):
            self.stop()
        ###############################################
    ...
    def set_batchsize(self,batch_size): #
        ...
        #pd.DataFrame([self.best_error]).to_csv(self.best_error_path,header=False,index=False)
    ...
    def set_lr(self,lr):
        ...
        print(lr)
        print('##############################################')
        self_past_lr = tf.keras.backend.get_value(self.model.optimizer.lr)
        tf.keras.backend.set_value(self.model.optimizer.lr,lr)
        pd.DataFrame([lr]).to_csv(self.lr_path,header=False,index=False)